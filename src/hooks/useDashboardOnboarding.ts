...
export function useDashboardOnboarding(): DashboardOnboardingState {
  ...
  useEffect(() => {
    ...
    const widgetHasButtonText = (w: any, buttonText: string): boolean => {
      if (!w) return false;
      if (
        (w?.type === "info-popup-button" || w?.type === "right-panel-button") &&
        w?.data?.buttonText === buttonText
      ) {
        return true;
      }
      if (w?.type === "widget-stack" && Array.isArray(w?.data?.widgets)) {
        return w.data.widgets.some((child: any) =>
          widgetHasButtonText(child, buttonText),
        );
      }
      return false;
    };

    const widgetHasType = (w: any, t: string): boolean => {
      if (!w) return false;
      if (w?.type === t) return true;
      if (w?.type === "widget-stack" && Array.isArray(w?.data?.widgets)) {
        return w.data.widgets.some((child: any) => widgetHasType(child, t));
      }
      return false;
    };
    ...
    const sanitizeWidget = (w: any): any => {
      if (!w) return w;
      if (w?.type === "widget-stack" && Array.isArray(w?.data?.widgets)) {
        const nextChildren = w.data.widgets
          .map((child: any) => sanitizeWidget(child))
          .filter((child: any) => {
            if (!child) return false;
            if (
              child?.type !== "info-popup-button" &&
              child?.type !== "right-panel-button"
            )
              return true;
            const payload =
            ...
            return !!panelType && !!buttonText && buttonText !== "View Details";
          });
        return { ...w, data: { ...w.data, widgets: nextChildren } };
      }
      return w;
    };

    const prependWidgets = (existingWidget: any, toPrepend: any[]) => {
      const cleanedExisting = sanitizeWidget(existingWidget);
      const additions = toPrepend.filter(
        (nw) => !widgetHasButtonText(cleanedExisting, nw?.data?.buttonText),
      );
      if (additions.length === 0) return cleanedExisting;
      ...
      if (
        cleanedExisting?.type === "widget-stack" &&
        Array.isArray(cleanedExisting?.data?.widgets)
      ) {
        return {
          ...cleanedExisting,
          data: {
            ...cleanedExisting.data,
            widgets: [...additions, ...cleanedExisting.data.widgets],
          },
        };
      }
      ...
      return {
        type: "widget-stack",
        data: { widgets: [...additions, cleanedExisting] },
      };
    };
      ...
      const cleaned = raw.filter((m: any) => {
        const isStandalone =
          ...
          ((m as any)?.widget?.type === "info-popup-button" ||
            (m as any)?.widget?.type === "right-panel-button");

        if (!isStandalone) return true;

        const buttonText = (m as any)?.widget?.data?.buttonText;
        const isKnownReference = referenceButtonTexts.includes(buttonText);
        const isBroken = !buttonText || buttonText === "View Details";

        // Remove BOTH known standalone buttons (they belong under other messages) and broken ones.
        return !(isKnownReference || isBroken);
      });

      return cleaned.map((m: any) => {
        if (!m || m.actor !== "assistant") return m;

        // Machine details step (contains machine-details-form widget)
        if (widgetHasType(m?.widget, "machine-details-form")) {
          const nextWidget = prependWidgets(m.widget, [
            machineConfigHelpWidget,
          ]);
          return nextWidget === m.widget ? m : { ...m, widget: nextWidget };
        }

        // Channel config step (contains channel-configuration-widget widget)
        if (widgetHasType(m?.widget, "channel-configuration-widget")) {
          const nextWidget = prependWidgets(m.widget, [
            channelConfigHelpWidget,
          ]);
          return nextWidget === m.widget ? m : { ...m, widget: nextWidget };
        }
    ...
    if (onboardingState) {
      try {
        ...
        if (state.shouldContinue) {
          ...
          const hasDeviceStatus = initialMsgs.some(
            (m) => (m as any)?.widget?.type === "device-status-widget",
          );
          if (!hasDeviceStatus && state.deviceId) {
            initialMsgs.push({
              ...
              actor: "assistant",
              message:
                "Creating your device with the provided configuration...",
              widget: {
  ...
  const addMessageFromCurrentStep = useCallback(
    (widget?: any) => {
      ...
      const baseWidget =
        currentStep.actor === "assistant"
          ? currentStep.widget || widget
          : undefined;
      ...
      const mergedWidget = (() => {
        ...
        if (
          baseWidget?.type === "widget-stack" &&
          Array.isArray(baseWidget?.data?.widgets)
        ) {
          return {
            ...baseWidget,
            data: {
        ...
        return {
          type: "widget-stack",
          data: { widgets: [...helpWidgets, baseWidget] },
        };
      })();
  ...
  const executeAction = useCallback(
    async (action: FlowStepAction, payload?: any) => {
      ...
      switch (action) {
        ...
        case "transfer-session": {
          try {
            ...
            const userSession = userSessionRaw
              ? JSON.parse(userSessionRaw)
              : null;
            const authenticatedUserId =
              userSession?.userId || "user_authenticated_123";
            ...
            console.warn(
              "Session transfer failed, proceeding optimistically",
              data,
            );
          } catch (e) {
            console.warn(
              "Session transfer error, proceeding optimistically",
              e,
            );
          }
          return true;
        ...
        case "subscribe-notifications": {
          try {
            ...
            console.warn(
              "Subscription API failed, proceeding optimistically",
              e,
            );
          }
        ...
        case "query-metrics": {
          try {
            const res = await fetch(
              "/api/v2/metrics/query?metrics=vibration,temperature&window=7d",
            );
            const data = await res.json();
        ...
        case "forecast-maintenance": {
          try {
            const res = await fetch("/api/v2/forecast/maintenance", {
              method: "POST",
            });
            const data = await res.json();
            console.log("🗓️ Forecast maintenance:", data);
            flowManager.updateContext({
              predictedMaintenanceInDays: data?.predictedInDays,
              predictedMaintenanceDate: data?.date,
            });
            return true;
          } catch (e) {
        ...
        case "create-test-ticket": {
          ...
          try {
            ...
            console.warn(
              "Failed creating demo ticket in /api/tickets (non-fatal)",
              e,
            );
          }
    ...
    async (input: string | any) => {
      ...
      try {
        ...
        const isYes = (s: string) =>
          /^(y|yes|yeah|yep|sure|ok|okay|continue)$/i.test(s.trim());
        const isNo = (s: string) => /^(n|no|nope|not now)$/i.test(s.trim());
        const isInduced = (s: string) =>
          /(induced|triggered|done|started|i did|i have induced|test condition)/i.test(
            s.trim(),
          );
        ...
        const fetchAssignableUsers = async (): Promise<AssignableUser[]> => {
          const ctx = flowManager.getContext();
          const invited = Array.isArray((ctx as any).invitedUsers)
            ? (ctx as any).invitedUsers
            : [];
          ...
          try {
            ...
            const data = await res.json();
            const apiUsers = Array.isArray(data?.state?.collaborators)
        ...
        const isShowUsersIntent = (s: string) =>
          /^show\s+me\s+users$|^show\s+users$|list\s+users|who\s+can\s+i\s+assign/i.test(
            s.trim(),
          );
        ...
        if (isPostLogin) {
          ...
          if (
            ...
            const users = [
              { name: email.split("@")[0] || "User", email, role: "Viewer" },
            ];
        ...
        if (typeof input === "string" && input && input.trim()) {
          ...
          if (isPostLogin) {
            ...
            if (postLoginStageRef.current === "awaiting-assignee") {
              ...
              if (isShowUsersIntent(text)) {
                ...
                await new Promise((resolve) => setTimeout(resolve, 500));
                addAssistantMessage(
                  `Here are the users you can assign tickets to:\n\n${list}`,
                );
                setIsProcessing(false);
                return;
              }
              ...
              if (!ticketId) {
                await new Promise((resolve) => setTimeout(resolve, 400));
                addAssistantMessage(
                  'I don’t have a ticket to assign yet. Please type "I see the ticket" first.',
                );
                setIsProcessing(false);
                return;
              }
              ...
              const users =
                assignableUsersRef.current.length > 0
                  ? assignableUsersRef.current
              ...
              const matches = users.filter((u) => {
                ...
                return tokens.some(
                  (t) =>
                    t &&
                    (t === normalizedInput || t.startsWith(normalizedInput)),
                );
              });
            ...
            if (
              ...
              if (!pending) {
                ...
                addAssistantMessage(
                  "No pending assignment found. Type a user name to assign the ticket.",
                );
              ...
              if (isNo(text)) {
                await new Promise((resolve) => setTimeout(resolve, 400));
                addAssistantMessage(
                  'Okay — not assigning it yet. Type a different name/email, or type "show me users".',
                );
                postLoginStageRef.current = "awaiting-assignee";
                setIsProcessing(false);
                return;
            ...
            if (
              ...
              if (isNo(text)) {
                await new Promise((resolve) => setTimeout(resolve, 500));
                addAssistantMessage(
                  "No problem — you can ask me to show the agentic workflow anytime.",
                );
                postLoginStageRef.current = "idle";
                setIsProcessing(false);
                return;
              ...
              addAssistantMessage(
                "Please provide the email address of the person you’d like to invite, and we’ll send them an invite so they can explore this machine and receive the appropriate alerts.",
                {
                  ...
                  data: {
                    ...
                    helperText:
                      "We'll send an invite email so they can access this machine.",
            ...
            if (
              ...
              if (isNo(text)) {
                await new Promise((resolve) => setTimeout(resolve, 500));
                addAssistantMessage(
                  "No problem — you can ask me to walk through the workflow anytime.",
                );
                postLoginStageRef.current = "idle";
                setIsProcessing(false);
                return;
          ...
          const assignMatch = userInput.match(
            /assign\s+(?:ticket\s+)?([a-z0-9-]+)\s+to\s+(.+)/i,
          );
          if (assignMatch) {
            addUserMessage(input);
            ...
            try {
              ...
              const res = await fetch("/api/tickets", {
                ...
                body: JSON.stringify({
                  action: "update",
                  id: ticketId,
                  updates: { owner: assignee },
                }),
              });
              const data = await res.json();
              if (data.ticket) {
                addAssistantMessage(
                  `I've assigned ticket ${ticketId} to ${assignee}.`,
                );
              } else {
                addAssistantMessage(
                  `I couldn't find ticket ${ticketId}. Please check the ID.`,
                );
              }
            } catch (e) {
          ...
          if (userInput.match(/invite|add user|team|collaborator/)) {
            ...
            if (userInput.includes("@")) {
              ...
              if (emails && emails.length > 0) {
                ...
                addAssistantMessage(
                  "I can help you invite team members. Who would you like to invite?",
                  { type: "user-invitation-form" },
                );
              }
            } else {
              addAssistantMessage(
          ...
          if (
            userInput.match(
              /recent ticket|latest ticket|show ticket|list ticket/,
            )
          ) {
            addUserMessage(input);
            try {
              ...
              if (data.tickets && data.tickets.length > 0) {
                const ticketList = data.tickets
                  .map(
                    (t: any) =>
                      `• **${t.related}**: ${t.summary} (${t.status})`,
                  )
                  .join("\n");
                addAssistantMessage(
                  `Here are the recent tickets:\n\n${ticketList}`,
                );
              } else {
          ...
          if (
            userInput.match(
              /see.*ticket|ticket.*appear|ticket.*visible|found.*ticket/i,
            )
          ) {
            addUserMessage(input);
            ...
            await new Promise((resolve) => setTimeout(resolve, 800));
            addAssistantMessage(
              `You can assign tickets directly through this chat interface — no need to navigate away. Type the name/email of the person you'd like to assign ticket **${tid}** to, or type "show me users" to see the list.`,
              {
                type: "info-grid",
                data: {
                  title: "APM Ticket Overview",
                  description:
                    "Snapshot of the current alert before assignment.",
                  fields: [
          ...
          if (
            userInput.match(
              /create ticket|open ticket|new ticket|issue|problem|repair/,
            )
          ) {
            addUserMessage(input);
            addAssistantMessage("Opening a new maintenance ticket for you...", {
              type: "ticket-creation-form",
            });
            await executeAction("create-test-ticket");
            const tid = flowManager.getContext().testTicketId;
            addAssistantMessage(
              `Draft ticket ${tid} created. Please fill in the details in the Tickets panel.`,
            );
            setIsProcessing(false);
            return;
          }
          ...
          if (userInput.match(/compare|sensor|graph|chart|metric/)) {
            addUserMessage(input);
            addAssistantMessage(
              "I've highlighted the sensor comparison view. You can toggle between Vibration, Temperature, and Pressure on the charts panel.",
            );
          ...
          const switchMatch = userInput.match(
            /(?:switch|change\s+graph|show)\s+(?!me\b)(.+)/i,
          );
          if (switchMatch) {
            ...
            if (!userInput.includes("ticket")) {
              ...
              const target = switchMatch[1]
                .replace(/graph|dashboard|sensor/gi, "")
                .trim();
              // Heuristic mapping
              let channel = "Speed";
              ...
              addAssistantMessage(
                ok
                  ? `Switched dashboard graphs to ${channel}.`
                  : `I couldn't switch to ${target}.`,
              );
              setIsProcessing(false);
              return;
            }
          ...
          if (text === "Show connection details") {
            ...
            if (ctx.mqttConnection) {
              ...
              addAssistantMessage(
                "Connection details are not available yet. Try after the device is created.",
              );
            }
            setIsProcessing(false);
            return;
          ...
          if (
            userInput.match(
              /restart|start.*new|onboard.*another|add.*another|fresh.*start/i,
            )
          ) {
            addUserMessage(input);
            await new Promise((resolve) => setTimeout(resolve, 500));
            addAssistantMessage("Ready to onboard a new machine?", {
              type: "restart-onboarding-widget",
              data: {
                message:
                  "Click the button below to start fresh with a new demo or live machine. Your current dashboard will remain accessible.",
              },
            });
            setIsProcessing(false);
            return;
          ...
          if (
            userInput.includes("onboard") ||
            userInput.includes("add machine") ||
            userInput.includes("add device")
          ) {
        ...
        if (isPostLogin) {
          ...
          if (typeof input === "string" && input && input.trim()) {
            ...
            await new Promise((resolve) => setTimeout(resolve, 800));
            addAssistantMessage(
              "I didn't catch that. You can ask me to invite users, show tickets, compare sensors, or create a ticket.",
            );
          }
          setIsProcessing(false);
          return;
        ...
        if (!currentStep) {
          ...
          if (typeof input === "string" && input && input.trim()) {
            ...
            const responseMessage: ChatMessage = {
              ...
              actor: "assistant",
              message:
                'I received your message. Try asking me to "add a machine" to get started with device onboarding!',
              timestamp: new Date(),
            };
        ...
        const isWidgetSubmission =
          typeof input === "object" && input && !input.userMessage;
        ...
        if (stepsProcessed >= maxSteps) {
          console.warn(
            "⚠️  Reached max steps limit, stopping to prevent infinite loop",
          );
        }
      } catch (error) {
  ...
  const addMessage = useCallback(
    (actor: "user" | "assistant", message: string, widget?: any) => {
